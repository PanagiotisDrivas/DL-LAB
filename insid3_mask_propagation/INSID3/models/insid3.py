...
from INSID3.utils.clustering import agglomerative_clustering, compute_cluster_prototypes
from INSID3.utils.data import build_transform, downsample_mask, load_image, load_mask
...
class INSID3(nn.Module):
    ...
    def set_reference(
        ...
        mask: str | Image.Image | torch.Tensor | None = None,
    ) -> None:
        ...
        """
        img_tensor, self._orig_ref_size = load_image(
            image, self._transform, self.device
        )
    ...
    def set_target(self, image: str | Image.Image | torch.Tensor) -> None:
        ...
        img_tensor, self._orig_tgt_size = load_image(
            image, self._transform, self.device
        )
    ...
    def segment(self) -> torch.Tensor:
        ...
        """
        if (
            self._ref_images is None
            or self._ref_masks is None
            or self._tgt_image is None
        ):
            raise RuntimeError(
                "segment() requires reference image(s), reference mask(s), and a target image."
            )
    ...
    def match(self, src_kps: torch.Tensor, use_debiased: bool = True) -> torch.Tensor:
        ...
        pred = self.predict_keypoint(
            self._ref_images, src_kps, self._tgt_image, use_debiased=use_debiased
        )
    ...
    @torch.no_grad()
    def predict_mask(
        self, ref_images: torch.Tensor, ref_masks: torch.Tensor, tgt_image: torch.Tensor
    ) -> torch.Tensor:
        ...
        # print(candidate_mask.shape) # Single instance A example
        # print(feat_tgt_deb.shape)  # 384 different instances A example
    ...
    def predict_keypoint(
        ...
        if not isinstance(ref_kps, torch.Tensor):
            ref_kps = torch.tensor(
                ref_kps, dtype=torch.float32, device=tgt_image.device
            )
        else:
    ...
    def _locate_candidates(
        ...
        for m, sim_m in enumerate(sim_maps):
            ...
            rows = best_idx // Ws
            cols = best_idx % Ws
            ref_mask_m = downsample_mask(ref_masks[m : m + 1], Hs, Ws).squeeze(
                0
            )  # (Hs, Ws)
    ...
    def _finalize_mask(
        self, mask: torch.Tensor, tgt_image: torch.Tensor
    ) -> torch.Tensor:
