...
def _load_mask(
    mask: str | Path | Image.Image | np.ndarray | torch.Tensor, size: tuple[int, int]
) -> np.ndarray:
    ...
    if mask_array.shape != size:
        ...
        mask_array = (
            np.array(mask_image.resize((size[1], size[0]), resample=Image.NEAREST)) > 0
        )
...
def _overlay_mask(
    image: np.ndarray, mask: np.ndarray, color: tuple[float, float, float], alpha: float
) -> np.ndarray:
...
def visualize_prediction_segmentation(
    ...
    *,
    ...
    reference_overlay = _overlay_mask(
        reference_np, reference_mask_np, color=(0.95, 0.25, 0.2), alpha=alpha
    )
    target_overlay = _overlay_mask(
        target_np, predicted_mask_np, color=(0.15, 0.8, 0.35), alpha=alpha
    )
...
def visualize_prediction_matching(
    ...
    *,
    ...
    def _setup_axis_with_grid(
        ax, img, title: str, width: int, height: int, y_ticks_side: str = "left"
    ) -> None:
        ax.imshow(img)
