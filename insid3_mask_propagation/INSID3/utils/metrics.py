...
class AverageMeter:
    ...
    def update(
        self, inter_b: torch.Tensor, union_b: torch.Tensor, class_id: torch.Tensor
    ) -> None:
    ...
    def compute_iou(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        iou = (
            ...
            / torch.max(torch.stack([self.union_buf, ones]), dim=0)[0]
        )
...
class Evaluator:
    ...
    def classify_prediction(
        ...
        tgt_ignore_idx: torch.Tensor | None = None,
...
class PCKEvaluator:
    ...
    def get_result(self):
        ...
        for alpha in self.alpha:
            ...
            per_cat_res[f"pck{alpha}"] = (
                per_cat_avg.mean() if len(cat_list) > 0 else 0.0
            )
    ...
    def save_result(self, save_file):
        ...
        for alpha in self.alpha:
            ...
            if len(catstring) == 0:
                catstring += (
                    " " * 12
                    + "".join([f"{category:<12}" for category in cat_list])
                    + "\n"
                )
                outstring += catstring
