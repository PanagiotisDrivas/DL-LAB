...
class DatasetISIC(Dataset):
    def __init__(self, datapath: str, shot: int, num: int = 600):
        ...
        self.ann_path = os.path.join(
            self.base_path, "ISIC2018_Task1_Training_GroundTruth"
        )
    ...
    def load_frame(self, tgt_name: str, ref_names: list[str]) -> tuple:
        ...
        ref_names = [
            os.path.join(self.ann_path, sid) + "_segmentation.png"
            for name, sid in zip(ref_names, ref_ids)
        ]
    ...
    def build_img_metadata_classwise(self) -> dict:
        ...
        for cat in self.categories:
            img_paths = sorted(
                [path for path in glob.glob("%s/*" % os.path.join(build_path, cat))]
            )
