...
class DatasetPASCALPart(Dataset):
    def __init__(self, datapath: str, fold: int, shot: int, box_crop: bool = True):
        ...
        self.json_file = os.path.join(
            datapath, "Pascal-Part/VOCdevkit/VOC2010/all_obj_part_to_image.json"
        )
        self.img_file = os.path.join(
            datapath, "Pascal-Part/VOCdevkit/VOC2010/JPEGImages/{}.jpg"
        )
        self.anno_file = os.path.join(
            datapath,
            "Pascal-Part/VOCdevkit/VOC2010/Annotations_Part_json_merged_part_classes/{}.json",
        )
        ...
        for i, obj in enumerate(self.cat_annos["object"]):
            for part in self.cat_annos["object"][obj]["part"]:
                if (
                    len(self.cat_annos["object"][obj]["part"][part]["train"]) > 0
                    and len(self.cat_annos["object"][obj]["part"][part]["val"]) > 0
                ):
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        batch = {
            ...
            "class_id": torch.tensor(
                self.class_ids[self.cat_part_name.index(class_sample)]
            ),
        }
    ...
    def sample_episode(self, idx: int):
        ...
        # Query selection
        while True:
            query_img_id = np.random.choice(
                self.cat_annos["object"][obj_n]["part"][part_n]["val"], 1, replace=False
            )[0]
        ...
        while len(support_img_ids) < self.shot:
            support_img_id = np.random.choice(
                self.cat_annos["object"][obj_n]["part"][part_n]["val"], 1, replace=False
            )[0]
        ...
        ref_imgs = [
            Image.open(self.img_file.format(sid)).convert("RGB")
            for sid in support_img_ids
        ]

        if self.box_crop:
            tgt_img = np.asarray(tgt_img)
            tgt_img = tgt_img[
                query_obj_box[1] : query_obj_box[3], query_obj_box[0] : query_obj_box[2]
            ]
            ...
            new_ref_imgs = []
            new_ref_masks = []
            for sup_img, sup_mask, sup_box in zip(
                ref_imgs, support_masks, support_boxes
            ):
                ...
                new_ref_masks.append(
                    sup_mask[sup_box[1] : sup_box[3], sup_box[0] : sup_box[2]]
                )
...
def build(args) -> DatasetPASCALPart:
    dataset = DatasetPASCALPart(
        datapath=args.data_root, fold=args.fold, shot=args.shots
    )
    return dataset
