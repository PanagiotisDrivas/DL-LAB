...
class DatasetSUIM(Dataset):
    def __init__(self, datapath: str, shot: int):
        ...
        self.img_metadata_classwise, self.num_images = (
    ...
    def load_frame(self, tgt_mask_path: str, ref_mask_paths: list[str]) -> tuple:
        ...
        ref_imgs = [
            Image.open(maskpath_to_imgpath(s_mask_path)).convert("RGB")
            for s_mask_path in ref_mask_paths
        ]
            ...
            mask_paths = sorted(
                [
                    ...
                    for path in glob.glob(
                        "%s/*" % os.path.join(self.base_path, "masks", cat)
                    )
                ]
            )
...
def build(args) -> DatasetSUIM:
    dataset = DatasetSUIM(
        datapath=os.path.join(args.data_root, "SUIM"), shot=args.shots
    )
    return dataset
