...
class DatasetPACOPart(Dataset):
    def __init__(self, datapath: str, fold: int, shot: int, box_crop: bool = True):
        ...
        self.class_ids_ori, self.cid2img, self.img2anno = (
    ...
    def build_img_metadata_classwise(self) -> tuple:
        ...
        test_cat_ids = [
            i
            for i in list(test_anno["cid2img"].keys())
            if len(test_anno["cid2img"][i]) > self.shot
        ]
        ...
        if self.fold != -1:
            class_ids = [
                train_cat_ids[self.fold + self.nfolds * v]
                for v in range(nclass_per_fold)
            ]
        else:
    ...
    def load_frame(self) -> tuple:
        ...
        for anno in tgt_annos:
            if anno["category_id"] == class_sample:
                ...
                if obj_id not in tgt_obj_dict:
                    tgt_obj_dict[obj_id] = {"obj_bbox": [], "segms": []}
                tgt_obj_dict[obj_id]["obj_bbox"].append(anno["obj_bbox"])
                tgt_obj_dict[obj_id]["segms"].append(
                    self.get_mask(anno["segmentation"], org_tgt_imsize)[None, ...]
                )
        ...
        while True:  # keep sampling until reference != target
            ref_sample = np.random.choice(self.cid2img[class_sample], 1, replace=False)[
                0
            ]
                ...
                for anno in ref_annos:
                    if anno["category_id"] == class_sample:
                        ...
                        if obj_id not in ref_obj_dict:
                            ref_obj_dict[obj_id] = {"obj_bbox": [], "segms": []}
                        ref_obj_dict[obj_id]["obj_bbox"].append(anno["obj_bbox"])
                        ref_obj_dict[obj_id]["segms"].append(anno["segmentation"])

                sel_ref_id = np.random.choice(
                    list(ref_obj_dict.keys()), 1, replace=False
                )[0]
        ...
        if self.box_crop:
            ...
            tgt_img = tgt_img[
                int(tgt_obj_bbox[1]) : int(tgt_obj_bbox[1] + tgt_obj_bbox[3]),
                int(tgt_obj_bbox[0]) : int(tgt_obj_bbox[0] + tgt_obj_bbox[2]),
            ]
            ...
            for sup_img, sup_mask, sup_box in zip(ref_imgs, ref_masks, ref_boxes):
                ...
                sup_img = sup_img[
                    int(sup_box[1]) : int(sup_box[1] + sup_box[3]),
                    int(sup_box[0]) : int(sup_box[0] + sup_box[2]),
                ]
