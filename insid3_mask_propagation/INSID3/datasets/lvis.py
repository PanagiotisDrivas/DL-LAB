...
class DatasetLVIS(Dataset):
    def __init__(self, datapath: str, fold: int, shot: int):
        ...
        self.nclass, self.class_ids_ori, self.img_metadata_classwise = (
    ...
    def build_img_metadata_classwise(self) -> dict:
        ...
        if self.fold != -1:
            class_ids = [
                val_cat_ids[self.fold + self.nfolds * v] for v in range(nclass_per_fold)
            ]
        else:
    ...
    def load_frame(self, idx: int) -> tuple:
        ...
        tgt_name = np.random.choice(
            list(self.img_metadata_classwise[class_sample].keys()), 1, replace=False
        )[0]
        ...
        for anno in tgt_annos:
            segms.append(
                self.get_mask(anno["segmentation"], org_tgt_imsize)[None, ...].float()
            )
        ...
        for ref_name, ref_pre_mask in zip(ref_names, ref_pre_masks):
            ...
            for pre_mask in ref_pre_mask:
                sup_masks.append(
                    self.get_mask(pre_mask, org_sup_imsize)[None, ...].float()
                )
