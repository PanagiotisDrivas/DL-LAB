...
class DatasetCOCO(Dataset):
    ...
    def build_img_metadata_classwise(self) -> dict:
        if self.fold != -1:
            with open(
                f"{self.base_path}/splits/{self.split}/fold{self.fold}.pkl", "rb"
            ) as f:
                img_metadata_classwise = pickle.load(f)
        else:
            split_meta_path = [
                f"{self.base_path}/splits/{self.split}/fold{str(idx)}.pkl"
                for idx in range(4)
            ]
            ...
            for meta in split_meta:
                for class_id in meta:
                    if (
                        len(img_metadata_classwise[class_id]) == 0
                        and len(meta[class_id]) > 0
                    ):
    ...
    def load_frame(self) -> tuple:
        ...
        for ref_name in ref_names:
            ref_imgs.append(
                Image.open(os.path.join(self.base_path, ref_name)).convert("RGB")
            )
