...
class DatasetISAID(Dataset):
    ...
    def read_mask(self, img_name: str) -> torch.Tensor:
        mask = torch.tensor(
            np.array(
                Image.open(
                    os.path.join(self.ann_path, img_name) + "_instance_color_RGB.png"
                )
            )
        )
        return mask
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        tgt_img, tgt_cmask, ref_imgs, ref_cmasks, org_tgt_imsize = self.load_frame(
            tgt_name, ref_names
        )

        tgt_mask, tgt_ignore_idx = self.extract_ignore_idx(
            tgt_cmask.float(), class_sample
        )
    ...
    def build_img_metadata(self) -> list:
        def read_metadata(split, fold_id):
            fold_n_metadata = os.path.join(
                f"{self.datapath}/splits/{split}/fold{fold_id}.txt"
            )
...
def build(args) -> DatasetISAID:
    dataset = DatasetISAID(
        datapath=os.path.join(args.data_root, "iSAID"), fold=args.fold, shot=args.shots
    )
    return dataset
